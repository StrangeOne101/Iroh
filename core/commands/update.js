...
function copyFiles(src, dest) {
  ...
  fs.readdirSync(src).forEach((file) => {
    ...
    if (fs.statSync(srcPath).isDirectory()) {
        fs.rmdirSync(srcPath, {recursive: true}); //Remove the directory to remove child files
...
module.exports = {
    ...
    run: async function(messageObj, channel, sender, args) {
        try {
            ...
            await downloadFile(url, zipPath).then(() => {
            ...
            await channel.send("Extracting zip file...")
            ...
            await channel.send("Replacing files...");
            ...
            await channel.send("✅ Update complete! Restarting bot...");
            process.exit(0);
        } catch (error) {
